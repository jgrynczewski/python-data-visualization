...
**Serie 9 Jupyter Notebooków wyjaśniających deployment Pythonowych aplikacji web od podstaw.**
...
### 09. API Gateway & Multi-Backend Routing
**Plik:** `09_api_gateway_routing.ipynb`

**Co się nauczysz:**
- **Czym jest API Gateway** - proste wyjaśnienie (analogia hotelu)
- **Multi-backend architecture** - różne backendy dla różnych klientów
- **Routing strategies** - path-based, header-based, subdomain
- **nginx routing** - praktyczna implementacja (docker-compose)
- **API Gateway przykłady** - AWS API Gateway, Kong (open-source)
- **Kubernetes Ingress** - routing w K8s
- **Kiedy co używać** - decision tree (nginx vs Kong vs AWS vs K8s)
- Praktyczny przykład end-to-end (Web + Mobile + Partner API)

**Analogia:** API Gateway = recepcja w hotelu (kieruje gości do pokoi/restauracji/SPA)

**Kluczowa koncepcja:** API Gateway ≠ Load Balancer ≠ Reverse Proxy (różne role!)
...
# Otwórz notebooki po kolei (01 → 09)
```
...
### API Gateway & Multi-Backend (Notebook 09):
✅ Czym jest API Gateway (nie mylić z load balancerem!)
✅ Multi-backend architecture (różne backendy dla Web/Mobile/Partners)
✅ Routing strategies (path-based, header-based, subdomain)
✅ nginx routing (praktyczne przykłady)
✅ API Gateway solutions (AWS, Kong, Kubernetes Ingress)
✅ Kiedy używać nginx vs Kong vs AWS API Gateway
✅ Praktyczny przykład (Web + Mobile + Partner API)
